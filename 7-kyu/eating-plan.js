...
function eatingPlan(b, p, meals){

  let onlyBowls = 0;
  let bowlsOrPlates = 0;
  
  for(let x=0; x<meals.length; x++){
    meals[x]===1? onlyBowls++ : bowlsOrPlates++;
  } 
  
  if (b-onlyBowls >= 0 && p-bowlsOrPlates >= 0) { return 0; }
  else {
   if (p-bowlsOrPlates < 0) { return Math.abs(p-bowlsOrPlates+(b-onlyBowls)); }
   if (b-onlyBowls < 0) { return Math.abs(b-onlyBowls); }
  }

}

console.log(eatingPlan(3,1,[1, 1, 1, 1]));
// Outputs: 1
